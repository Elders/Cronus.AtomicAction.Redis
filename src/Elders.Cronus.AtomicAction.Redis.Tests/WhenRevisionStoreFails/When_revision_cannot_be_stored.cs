...
namespace Elders.Cronus.AtomicAction.Redis.Tests.WhenRevisionStoreFails
{
    ...
    public class When_revision_store_cannot_store_a_revision
    {
        Establish context = () =>
        {
            ...
            A.CallTo(() => revisionStore.GetRevision(id)).Returns(new Userfull.Result<int>(0));
            A.CallTo(() => revisionStore.SaveRevision(id, A<int>.Ignored)).Throws(new Exception(message));
